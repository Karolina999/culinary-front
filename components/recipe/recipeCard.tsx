...
interface RecipeProps {
  recipe: Recipe;
  buttonToEdit?: boolean;
}
const RecipeCard = ({ recipe, buttonToEdit }: RecipeProps) => {
  ...
  return (
    <div style={{ width: "250px", position: "relative" }}>
      ...
      >
        <div
          style={{ width: "250px" }}
          className="mx-auto caladea-font shadow "
        >
          <Image
            src={
      ...
      {buttonToEdit && (
        <div style={{ position: "absolute", top: "10px", right: "10px" }}>
          <Button
            icon="pi pi-pencil"
            className="p-button-sm p-button-success bg-success border-success p-1"
            onClick={() =>
              (window.location.href = `/przepisy/edytuj/${recipe.id}`)
            }
          />
        </div>
      )}
